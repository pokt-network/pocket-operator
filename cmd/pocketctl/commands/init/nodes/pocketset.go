...
// getPocketSetManifest returns the sample PocketSet manifest
// based upon API Version input.
func getPocketSetManifest(i *cmdinit.InitSubCommand) (string, error) {
	apiVersion := i.APIVersion
	if apiVersion == "" || apiVersion == "latest" {
		return nodes.PocketSetLatestSample, nil
	}

	// generate a map of all versions to samples for each api version created
	manifestMap := map[string]string{
		"v1alpha1": v1alpha1pocketset.Sample(i.RequiredOnly),
...
func InitPocketSet(i *cmdinit.InitSubCommand) error {
	manifest, err := getPocketSetManifest(i)
	if err != nil {
		return fmt.Errorf("unable to get manifest for PocketSet; %w", err)
	}
