...
func NewPocketValidatorSubCommand(parentCommand *cobra.Command) {
	generateCmd := &cmdgenerate.GenerateSubCommand{
		...
		GenerateFunc:          GeneratePocketValidator,
		UseCollectionManifest: true,
		CollectionKind:        "PocketSet",
		UseWorkloadManifest:   true,
		WorkloadKind:          "PocketValidator",
	}

	generateCmd.Setup()
}

// GeneratePocketValidator runs the logic to generate child resources for a
// PocketValidator workload.
func GeneratePocketValidator(g *cmdgenerate.GenerateSubCommand) error {
	...
	generateFuncMap := map[string]generateFunc{
		"v1alpha1": v1alpha1pocketvalidator.GenerateForCLI,
